...
def augment(data, number, patch_size):
	...
	with tf.device('/device:cpu:0'):
		for i in range(number_set):
			...
			resam_data2 = tf.image.resize(resam_data, [patch_size, patch_size], method='nearest')
			agu_data = tf.image.rot90(resam_data2, k=idx)
			idx = idx + 1
			if i == 0:
				new_data = tf.image.resize(data, [patch_size, patch_size], method='nearest')
...
def scale_resize(ref_data, sen_data, patch_size):
	sample, row, col, band = ref_data.shape
	center = row / 2
	patch_st = int((center - 1) - patch_size / 2)
	patch_end = int((center - 1) + patch_size / 2)
	reref_data = ref_data[:, patch_st:patch_end, patch_st:patch_end, :]
	resen_data = sen_data[:, patch_st:patch_end, patch_st:patch_end, :]

	return reref_data, resen_data


def save_model_loss(train_loss, val_loss, output_dir):
	train_loss1 = train_loss
	val_loss2 = val_loss
	model_loss = np.zeros((50, 2))
...
def concat_patch(ref_data, sen_data):
	sample, row, col, band = ref_data.shape
	train_data = np.zeros((sample, row, col, band * 2))

	for i in range(len(train_data)):
		train_data[i, :, :, 0:3] = ref_data[i, :, :, :]
		train_data[i, :, :, 3:6] = sen_data[i, :, :, :]
